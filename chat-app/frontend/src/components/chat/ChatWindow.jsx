...
export default function ChatWindow() {
  ...
  const { activeChat, messages, isLoadingMessages, typingUsers, markRead } = useChatStore();
  ...
  const renderAvatar = (sender, size = "w-8 h-8") => (
    <div className={`${size} rounded-full bg-primary-600 flex items-center justify-center flex-shrink-0 overflow-hidden`}>
      {sender?.avatar
      ...
      <div key={msg._id} className={`flex gap-2 group message-enter ${isMine ? "flex-row-reverse" : "flex-row"} ${isSameGroup ? "mt-1" : "mt-3"}`}>
        ...
        <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-xs lg:max-w-md`}>
          ...
          {showAvatar && !isSameGroup && (
            <span className="text-xs text-primary-400 mb-1 ml-1">{msg.sender?.name}</span>
          )}

          {/* Reply preview */}
          {msg.replyTo && !msg.isDeleted && (
            <div className={`text-xs px-3 py-1.5 rounded-xl mb-1 border-l-2 border-primary-500 bg-dark-800/80 ${isMine ? "text-right" : "text-left"}`}>
              <span className="text-primary-400 block font-medium">{msg.replyTo?.sender?.name || "Unknown"}</span>
          ...
          <div className="relative">
            {msg.isDeleted ? (
              ...
              <a href={msg.fileUrl} target="_blank" rel="noreferrer">
                <img src={msg.fileUrl} alt="Shared image" className="max-w-xs rounded-2xl cursor-pointer hover:opacity-90 transition-opacity" />
              </a>
            ) : msg.type === "file" ? (
              <a href={msg.fileUrl} target="_blank" rel="noreferrer"
            ...
            {!msg.isDeleted && (
              <div className={`absolute top-1/2 -translate-y-1/2 ${isMine ? "-left-20" : "-right-20"} hidden group-hover:flex items-center gap-1`}>
                <button onClick={() => setReplyTo(msg)} className="p-1.5 rounded-lg bg-dark-800 hover:bg-dark-700 text-slate-400 hover:text-white transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                  </svg>
                </button>
                {isMine && (
                  <button onClick={() => handleDeleteMessage(msg._id)} className="p-1.5 rounded-lg bg-dark-800 hover:bg-red-500/20 text-slate-400 hover:text-red-400 transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className={`flex items-center gap-1 mt-1 ${isMine ? "flex-row-reverse" : "flex-row"}`}>
            ...
            {isMine && !msg.isDeleted && (
              <svg className={`w-3.5 h-3.5 ${isRead ? "text-primary-400" : "text-slate-600"}`} fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Chat header */}
      <div className="flex items-center gap-3 px-6 py-4 bg-dark-900 border-b border-slate-800 flex-shrink-0">
        <div className="relative">
